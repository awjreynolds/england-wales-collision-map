...
const copy = {
  ...
  subtitle: 'A national evidence view for reported injury collisions, persistent collision sites and school proximity screens.',
};

type LoadState = { manifest: DatasetManifest | null; error: string | null };
type SummaryState = { selected: SummaryMetrics | null; viewport: SummaryMetrics | null; viewportAnalysisKey: string | null; datasetVersion: string | null; loading: boolean; stale: boolean; error: string | null };
...
const AnalysisGroupDetail = ({ group, onClose }: { group: AnalysisGroup; onClose: () => void }) => {
  ...
  return <section className="analysis-selection" aria-labelledby="analysis-selection-heading">
    <div className="section-heading-row"><div><p className="section-kicker">Selected persistent collision site</p><h3 id="analysis-selection-heading">Group inspection</h3></div><button className="text-button" type="button" onClick={onClose}>Close</button></div>
...
export const App = () => {
  ...
  const [summary, setSummary] = useState<SummaryState>({ selected: null, viewport: null, viewportAnalysisKey: null, datasetVersion: null, loading: false, stale: true, error: null });
  ...
  const analysisDebounce = useRef<number | null>(null);
  const selectedAnalysisId = useRef<string | null>(null);
  ...
  const analysisOptions = useMemo(() => activeBBox ? paramsFor(filters, activeBBox, undefined, radius, harm, schoolDistance, selectedSchoolId) : null, [activeBBox, filters, harm, radius, schoolDistance, selectedSchoolId]);
  const analysisOptionsKey = analysisOptions ? queryString(analysisOptions) : null;
  const analysisBlocked = !summary.stale && (summary.viewport?.collisions ?? 0) > 10_000;
  const clearSelectedAnalysisGroup = () => { selectedAnalysisId.current = null; setSelectedAnalysisGroup(null); };
  useEffect(() => {
    if (!manifest || !requestOptions) return undefined;
    const controller = new AbortController(); const sequence = ++requestSequence.current; setSummary((current) => ({ ...current, viewportAnalysisKey: null, loading: true, stale: true, error: null }));
    const selectedOptions = paramsFor(filters, undefined, undefined, radius, harm, schoolDistance, selectedSchoolId);
    const viewportAnalysisKey = queryString(paramsFor(filters, requestOptions.bbox, undefined, radius, harm, schoolDistance, selectedSchoolId));
    Promise.all([loadView(requestOptions, controller.signal), loadSummary(selectedOptions, controller.signal), loadSummary(requestOptions, controller.signal)]).then(([viewResponse, selectedResponse, viewportResponse]) => {
      ...
      if (viewResponse.datasetVersion !== manifest.datasetVersion || selectedResponse.datasetVersion !== manifest.datasetVersion || viewportResponse.datasetVersion !== manifest.datasetVersion) {
        setSummary((current) => ({ ...current, viewportAnalysisKey: null, loading: false, stale: true, error: 'The dataset changed while this view was loading. Reload the page to synchronise the map.' }));
        return;
      }
      setView(viewResponse.data); setSummary({ selected: selectedResponse.data.metrics, viewport: viewportResponse.data.metrics, viewportAnalysisKey, datasetVersion: viewResponse.datasetVersion, loading: false, stale: false, error: null });
    }).catch((error: unknown) => { if (sequence === requestSequence.current && (error as Error).name !== 'AbortError') setSummary((current) => ({ ...current, viewportAnalysisKey: null, loading: false, stale: true, error: error instanceof Error ? error.message : 'The national view could not be loaded.' })); });
    return () => controller.abort();
  }, [manifest, requestOptions, filters, radius, harm, schoolDistance, selectedSchoolId, mapRefresh]);
  useEffect(() => {
    const sequence = ++analysisSequence.current;
    if (analysisDebounce.current !== null) window.clearTimeout(analysisDebounce.current);
    analysisDebounce.current = null;
    analysisController.current?.abort();
    analysisController.current = null;
    setAnalysisRunning(false);
    if (!analysisEnabled || analysisBlocked) clearSelectedAnalysisGroup();
    setAnalysis({ groups: [], coverage: [], provenance: [], inputRecords: null, edgeWarning: false, error: null });
    if (!analysisEnabled || !manifest || !analysisOptions || !analysisOptionsKey || analysisBlocked || summary.stale || summary.viewportAnalysisKey !== analysisOptionsKey) return undefined;
    setAnalysisRunning(true);
    analysisDebounce.current = window.setTimeout(() => {
      if (sequence !== analysisSequence.current) return;
      const controller = new AbortController();
      analysisController.current = controller;
      loadAnalysis(analysisOptions, controller.signal).then((response) => {
        if (sequence !== analysisSequence.current) return;
        if (response.datasetVersion !== manifest.datasetVersion) {
          clearSelectedAnalysisGroup();
          setAnalysis({ groups: [], coverage: [], provenance: [], inputRecords: null, edgeWarning: false, error: 'The dataset changed while loading persistent collision sites. Reload the page to synchronise the map.' });
          setAnalysisRunning(false);
          return;
        }
        const nextSelectedGroup = selectedAnalysisId.current ? response.data.groups.find((group) => group.id === selectedAnalysisId.current) ?? null : null;
        if (selectedAnalysisId.current && !nextSelectedGroup) selectedAnalysisId.current = null;
        setSelectedAnalysisGroup(nextSelectedGroup);
        setAnalysis({ groups: response.data.groups, coverage: response.data.schoolCoverage, provenance: response.data.schoolCoverageProvenance ?? [], inputRecords: response.data.inputRecords, edgeWarning: response.data.edgeWarning, error: null });
        setAnalysisRunning(false);
      }).catch((error: unknown) => {
        if (sequence !== analysisSequence.current || (error as Error).name === 'AbortError') return;
        clearSelectedAnalysisGroup();
        setAnalysis((current) => ({ ...current, error: error instanceof ApiRequestError && error.code === 'analysis_limit_exceeded' ? 'Persistent collision sites are paused until the map or filters narrow below 10,000 matching records.' : error instanceof Error ? error.message : 'Persistent collision site analysis could not be completed.' }));
        setAnalysisRunning(false);
      });
    }, 250);
    return () => {
      ...
      analysisController.current?.abort();
      analysisController.current = null;
      if (analysisSequence.current === sequence) analysisSequence.current += 1;
    };
  }, [analysisBlocked, analysisEnabled, analysisOptions, analysisOptionsKey, manifest, summary.stale, summary.viewportAnalysisKey]);
  ...
  const handleCollision = (id: string, point: [number, number]) => { detailController.current?.abort(); const sequence = ++detailSequence.current; const controller = new AbortController(); detailController.current = controller; clearSelectedAnalysisGroup(); setSelectedCollisionId(id); setSelectedPoint(point); setDetail(null); setDetailLoading(true); loadCollisionDetail(id, controller.signal).then((response) => { if (sequence === detailSequence.current) setDetail(response.data); }).catch(() => undefined).finally(() => { if (sequence === detailSequence.current) setDetailLoading(false); }); };
  const handleAnalysisGroup = (group: AnalysisGroup) => { detailController.current?.abort(); detailSequence.current += 1; setDetailLoading(false); setDetail(null); setSelectedCollisionId(null); setSelectedPoint(null); selectedAnalysisId.current = group.id; setSelectedAnalysisGroup(group); const span = Math.max(0.025, (radius / 111_000) * 3); setFocusBBox({ west: group.anchor.longitude - span, east: group.anchor.longitude + span, south: group.anchor.latitude - span, north: group.anchor.latitude + span }); };
  ...
  if (loadState.error) return <main className="state-page"><div className="state-card error-state"><h1>England &amp; Wales Observatory</h1><p>{loadState.error}</p><p>Check the published data files and reload.</p></div></main>;
  ...
  const analysisPauseReason = summary.stale
    ? summary.loading ? 'Persistent collision sites are waiting for the current map data to finish loading.' : 'Persistent collision sites are paused while the current map data is unavailable.'
    : blocked ? 'Persistent collision sites are paused above 10,000 matching records and will resume automatically when the result is narrowed.'
      : null;
  const analysisStatus = !analysisEnabled
    ? 'Persistent collision sites are off.'
    : blocked
      ? 'Persistent collision sites are paused above 10,000 matching records; narrow the map or filters to resume automatically.'
      : analysisRunning
        ? 'Updating persistent collision sites…'
        : analysis.error ? `Persistent collision site analysis failed: ${analysis.error}`
          : analysis.groups.length ? `${format(analysis.groups.length)} persistent collision site${analysis.groups.length === 1 ? '' : 's'} shown.`
            : summary.stale ? summary.loading ? 'Waiting for the current map result before updating persistent collision sites.' : view ? 'Persistent collision sites are paused while the current map result is unavailable.' : 'Persistent collision sites are waiting for map data.'
              : 'No persistent collision sites found for the current map extent and filters.';
  const aggregateNote = view?.mode === 'aggregates'
    ? zoom >= 11 ? 'Dense view grouped · zoom in for individual collisions' : 'Aggregated cells · click for details and zoom'
    : '\u00a0';
  return <div className="app-shell"><header className="app-header"><div className="header-inner"><div className="brand-mark" aria-hidden="true"><span>EW</span><i /></div><div><p className="eyebrow">National road safety evidence</p><h1>{copy.title}</h1><p className="subtitle">{copy.subtitle}</p></div><div className="header-status"><span className="status-dot" aria-hidden="true" />{summary.loading ? 'Updating' : 'Current dataset'}<br /><strong>{manifest.years.at(-1) ?? 'Latest available year'}</strong><nav className="header-links" aria-label="Project links"><a href="https://awjreynolds.github.io/">All projects</a><a href="https://github.com/awjreynolds/england-wales-collision-map">Source</a><a href="#method">Method</a></nav></div></div></header><main className="workspace"><section className="map-column" aria-label="England and Wales collision map"><div className="map-toolbar"><div><span className="map-toolbar-label">{bboxLabel(activeBBox)}</span><strong>{matchingCount === null ? 'Updating…' : `${format(matchingCount)} matching collisions`}</strong><small className="aggregate-note">{aggregateNote}</small></div><div className="map-toolbar-actions"><label className="analysis-toggle"><input type="checkbox" checked={analysisEnabled} onChange={(event) => setAnalysisEnabled(event.target.checked)} aria-describedby="map-analysis-status" /><span>Persistent collision sites</span></label><button className="map-reset" type="button" onClick={() => { setBBox(manifest.extent); setFocusBBox(manifest.extent); setResetSignal((value) => value + 1); }}>National view</button></div></div><div className="map-frame"><div className="map-analysis-status-panel"><p id="map-analysis-status" className={`map-analysis-status ${analysis.error || summary.error ? 'error' : ''}`} aria-live="polite">{analysisStatus}</p>{analysisPauseReason && !blocked && <p className="map-analysis-disabled">{analysisPauseReason}</p>}{summary.error && !summary.loading && <button className="map-retry" type="button" onClick={() => setMapRefresh((value) => value + 1)}>Retry map request</button>}</div><MapView view={view} manifest={manifest} initialBBox={initial.bbox ?? null} initialZoom={initial.zoom ?? 5} schools={schools} analysisGroups={analysis.groups} resetSignal={resetSignal} focusBBox={focusBBox} selectedCollisionId={selectedCollisionId} selectedPoint={selectedPoint} selectedAnalysisGroup={selectedAnalysisGroup} detail={detail} onPopupClose={() => { detailController.current?.abort(); detailSequence.current += 1; setDetailLoading(false); setDetail(null); setSelectedCollisionId(null); setSelectedPoint(null); clearSelectedAnalysisGroup(); }} onBoundsChange={(next, nextZoom) => { setBBox((current) => sameBBox(current, next) ? current : next); setZoom((current) => Math.abs(current - nextZoom) < 0.01 ? current : nextZoom); }} onAggregateClick={(next) => { setBBox(next); setFocusBBox(next); }} onCollisionClick={handleCollision} onAnalysisGroupClick={handleAnalysisGroup} onSchoolClick={(school) => { setSchoolQuery(school.name); setSelectedSchoolId(school.id); }} />{summary.stale && <div className="map-stale-banner">{summary.error ? view ? `Previous map result shown · ${summary.error}` : `Map data unavailable · ${summary.error}` : 'Updating map result…'}</div>}<div className="map-legend"><span>Map key</span><span><i className="legend-cluster" />Aggregate cell</span><span><i className="legend-hotspot" />Persistent collision site</span><span><i className="legend-fatal" />Fatal</span><span><i className="legend-serious" />Serious</span><span><i className="legend-slight" />Slight</span><span><i className="legend-school" />School · S2 = 2 schools</span></div></div></section><aside className="sidebar"><div className="sidebar-scroll"><section className="quality-note"><div className="quality-icon">i</div><div><h2>Read this map as evidence</h2><p>Reported STATS19 injury collisions record reported harm, not every incident. The school screen informs Safe System investigation; it does not infer routes, attendance, exposure or future KSI probability.</p>{manifest.qualityNotices.slice(0, 2).map((notice) => <p key={notice.id}><strong>{notice.title}:</strong> {notice.text}</p>)}</div></section><Metrics metrics={summary.selected} label="Selected national scope" updating={summary.loading} stale={summary.stale} /><Metrics metrics={summary.viewport} label="Current map extent" updating={summary.loading} stale={summary.stale} />{summary.error && <p className="limit-warning">Current totals could not be refreshed: {summary.error}</p>}<PlaceSearch onSearch={handlePlace} status={placeStatus} /><Filters manifest={manifest} filters={filters} onChange={handleFiltersChange} onPreset={() => handleFiltersChange({ ...filters, country: 'England', authorities: WEST_OF_ENGLAND })} onReset={() => { handleFiltersChange(DEFAULT_FILTERS); setSelectedSchoolId(undefined); setSchoolQuery(''); setSchoolSearch(''); setHarm('all'); setSchoolDistance(undefined); clearSelectedAnalysisGroup(); }} /><section className="search-panel school-search"><p className="section-kicker">School catalogue</p><h2>School proximity screen</h2><form onSubmit={(event) => { event.preventDefault(); const nextQuery = schoolQuery.trim(); setSchoolSearch(nextQuery); setSchoolOffset(0); setSchoolRefresh((value) => value + 1); setSchoolError(null); setSchoolHasMore(false); setSchools([]); setShowAllSchools(false); setSelectedSchoolId(undefined); }}><div className="search-row"><input aria-label="School name" value={schoolQuery} onChange={(event) => setSchoolQuery(event.target.value)} placeholder="Search a school name" /><button type="submit">Search</button></div></form>{schoolLoading && <p className="inline-note" aria-live="polite">Loading school matches…</p>}{schoolError && <p className="limit-warning" role="alert">{schoolError}</p>}{selectedSchoolId && <p className="inline-note">Selected school filter active. Clear the search and select a result to remove it.</p>}{(showAllSchools ? schools : schools.slice(0, 6)).map((school) => <button className={`school-result ${selectedSchoolId === school.id ? 'selected' : ''}`} key={school.id} type="button" onClick={() => { setSelectedSchoolId(school.id); setSchoolQuery(school.name); setFocusBBox({ west: school.longitude - .03, east: school.longitude + .03, south: school.latitude - .02, north: school.latitude + .02 }); setBBox({ west: school.longitude - .03, east: school.longitude + .03, south: school.latitude - .02, north: school.latitude + .02 }); }}>{school.name}<small>{school.country} · {school.phase ?? 'phase not recorded'}</small></button>)}{schools.length > 6 && <button className="text-button school-results-toggle" type="button" onClick={() => setShowAllSchools((value) => !value)}>{showAllSchools ? 'Show fewer matches' : `Show all ${format(schools.length)} loaded matches`}</button>}{schoolSearch && schoolHasMore && <button className="text-button school-results-more" type="button" disabled={schoolLoading} onClick={() => { setSchoolOffset((value) => value + SCHOOL_PAGE_SIZE); setShowAllSchools(true); }}>{schoolLoading ? 'Loading more…' : 'Load more matches'}</button>}{schoolSearch && !schoolLoading && !schoolError && !schools.length && <p className="inline-note">No schools matched “{schoolSearch}”.</p>}</section><AnalysisPanel groups={analysis.groups} coverage={analysis.coverage} provenance={analysis.provenance} inputRecords={analysis.inputRecords ?? summary.viewport?.collisions ?? null} edgeWarning={analysis.edgeWarning} radius={radius} harm={harm} schoolDistance={schoolDistance} selectedGroup={selectedAnalysisGroup} onGroupSelect={handleAnalysisGroup} onGroupClose={() => { clearSelectedAnalysisGroup(); }} onRadius={(value) => { setRadius(value); }} onHarm={(value) => { setHarm(value); }} onSchoolDistance={(value) => { setSchoolDistance(value); }} blocked={Boolean(analysisPauseReason)} error={analysis.error} /><DetailPanel detail={detail} loading={detailLoading} onClose={() => { detailController.current?.abort(); detailSequence.current += 1; setDetailLoading(false); setDetail(null); setSelectedCollisionId(null); setSelectedPoint(null); }} /><details id="method" className="provenance-panel"><summary>About the data and method</summary><div className="provenance-content"><p>{manifest.source.publisher} · {manifest.source.dataset}. Generated {new Date(manifest.generatedAt).toLocaleDateString('en-GB')}; {format(manifest.collisionCount)} mapped collision rows.</p><p>Persistent collision sites use a deterministic anchor radius and require three collisions across two calendar years. The bounded analysis is capped at 10,000 matching records. School points come from the published England GIAS and Welsh DataMapWales layers; Welsh independent schools without authoritative public coordinates are excluded and disclosed in the coverage result.</p><p>Authority codes are source-recorded. Selecting a single code can cover only part of a 2021–25 series when an authority changed; location browsing uses the full spatial extent independently of administrative change.</p><p>See <a href="https://www.gov.uk/government/statistical-data-sets/road-safety-open-data" target="_blank" rel="noreferrer">DfT STATS19</a>, <a href="https://get-information-schools.service.gov.uk/" target="_blank" rel="noreferrer">DfE GIAS</a> and <a href="https://datamap.gov.wales/" target="_blank" rel="noreferrer">DataMapWales</a>.</p></div></details><p className="footer-note">Dataset {manifest.datasetVersion} · {manifest.scope}. View totals are exact for the current browser query; changing the map extent loads a new bounded result.</p></div></aside></main></div>;
};
