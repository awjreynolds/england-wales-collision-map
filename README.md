...
The source investigation selected Department for Transport **STATS19** collision, vehicle and casualty records for the latest five complete final years available to this build: **2021–2025**. DfT's open-data page identifies 2025 as the latest final validated year and records that it was added on 30 July 2026. The generated manifest is authoritative for the actual included years. The shipped snapshot contains **8,033 mapped collisions** and **9,731 linked casualties**: 91 fatal, 1,295 serious and 6,647 slight collisions, with 97 fatalities, 1,396 serious casualties and 1,493 KSI casualties. All regional collision rows in this window have usable coordinates.
...
The map overlay reports collision and casualty severity for the records currently visible in the map. It updates after panning, zooming or changing filters; the regional summary remains the all-matching-record total.

Map clusters are only a zoom-dependent display aid. Persistent-location analysis instead uses a configurable ground-distance radius, initially 100m, with a minimum of three collisions across two distinct selected calendar years. It is an exploratory grouping method, not an official assessment of sites or roads. Filtering to one calendar year can correctly produce no persistent locations. Records are sorted by stable collision ID; each first unassigned record anchors a group and absorbs still-unassigned records within the haversine radius. Every member is within the radius of its anchor, but the group diameter can reach twice that radius. Groups are exploratory and order-dependent, with the fixed ID order making a given selection repeatable. See [the method evidence](docs/research/quality-and-method.md) and [verification record](docs/verification.md).
