...
Whenever the map response contains individual collision records, the map shows severity-coloured markers at their recorded coordinates without regrouping them into numbered circles. Broad views that require aggregate cells retain display grouping with a minimum six-pixel gap between circle outlines. Display grouping does not change exact totals. Persistent collision sites and school points remain distinct from individual collision markers in the close-up view; coincident records remain selectable.
